...
from src.utils import create_dir, plot_box_plot_hist_plot


class ParagraphAnalyzer(object):
    def __init__(self, dataframe, in_column, out_dir="./"):
        self.paragraphs_df = dataframe
    ...
    def tokenize_sentences(self):
        self.paragraphs_df["sentences"] = self.paragraphs_df[self.in_column].apply(
            sent_tokenize)

    def tokenize_words(self):
        self.paragraphs_df["words"] = self.paragraphs_df[self.in_column].apply(
            word_tokenize)
    ...
    def add_paragraph_chars_count(self):
        self.paragraphs_df["paragraph_chars_count"] = self.paragraphs_df[self.in_column].str.len(
        )
    ...
    def characters_per_paragraph_histogram(self, figsize=(4, 4), dpi=300, save_flag=False):
        fig = plot_box_plot_hist_plot(
            self.paragraphs_df,
            column="paragraph_chars_count",
            title="Characters per Paragraph",
            figsize=figsize,
            dpi=dpi,
            save_flag=save_flag,
    ...
    def avg_word_len_per_paragraph_histogram(self, figsize=(4, 4), dpi=300, save_flag=False):
        fig = plot_box_plot_hist_plot(
            ...
            title="Average Word Length per Paragraph",
            figsize=figsize,
            dpi=dpi,
            save_flag=save_flag,
            file_path=os.path.join(
    ...
    def get_top_k_ngrams(self, n=2, top_k=10):
        vec = CountVectorizer(ngram_range=(n, n)).fit(
            self.paragraphs_df[self.in_column])
        bag_of_words = vec.transform(self.paragraphs_df[self.in_column])
